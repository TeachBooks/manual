...
Using the terminal, navigate to the root folder of your project and run `pixi install`. This will install all necessary software and packages - these are installed in the project folder in `.pixi/`.
...
## Add dependencies
In the pixi.toml file, we included some of the most used depencies. If you want to add packages, use `pixi add <packages>` or if it is only available through pypi, use `pixi add --pypi <packages>`. Pixi adds the dependency to your project and updates the environment.
